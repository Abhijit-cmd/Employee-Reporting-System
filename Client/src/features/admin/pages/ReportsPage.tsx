      ...
      <div className="vr-drawer">
        ...
        <div className="vr-drawer-header">
          <div className="vr-drawer-title-wrap">
            ...
            <span
            className={`status-badge rp-badge ${
              STATUS_STYLES[report.reportStatus?.statusName as ReportStatus]
            }`}
          >
          ...
          <div className="vr-drawer-section">
            ...
            <div className="vr-section-body">
             <ReadonlyNumber
                ...
                value={report.customersRegistered}
                required
              />

              <ReadonlyNumber
                label="No of Supplier Registration :"
                value={report.suppliersRegistered}
                required
              />

              <ReadonlyNumber
                label="Name of Products /Brand added :"
                value={report.newBrandProducts}
                required
              />

              <ReadonlyNumber
                label="New Success Stories :"
                value={report.successStories}
              />

              <ReadonlyNumber
                label="No of Visits to new site :"
                value={report.websiteVisitors}
              />
            </div>
          </div>
...
// ── Reports list ──────────────────────────────────────────────────────────────
function ReportsList({ onView }: { onView: (r: Report) => void }) {
              ...
              {rows.length === 0 ? (
                ...
                <tr key={r.id}>
                  ...
                  <td>
                    <span
                      className={`status-badge rp-badge ${
                        STATUS_STYLES[r.reportStatus?.statusName as ReportStatus]
                      }`}
                    >
...
export default function ReportsPage({ onNavigate: _onNavigate }: Props) {
  ...
  return (
    <>
      ...
      {viewReport && <ViewReportDrawer report={viewReport} onClose={() => setViewReport(null)} />}
    </>
  )
}
