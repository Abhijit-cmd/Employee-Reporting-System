...
import { IconFileText, IconClock, IconMegaphone } from '../../../components/icons'
...
interface Item {
  id: number; icon: string; color: string; iconColor: string;
  ...
  useEffect(() => {
    apiFetch<Report[]>('/api/reports/my-reports')
      .then(reports => {
        ...
        setItems(list.map(r => ({
          id:        r.id,
          icon:      ['Pending', 'Draft'].includes(r.reportStatus?.statusName ?? '') ? 'pending' : 'report',
          color:     '#d1fae5',
          iconColor: '#10b981',
    ...
    <div className="card">
      ...
      <div className="card-body" style={{ paddingTop: 4, paddingBottom: 4 }}>
        ...
        {!loading && items.length === 0 && (
          <p style={{ padding: 16, color: 'var(--text-muted)' }}>No notifications yet.</p>
        )}
