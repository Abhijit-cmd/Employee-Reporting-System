...
export function ThemeModeCard() {
  ...
  return (
    <div className="card st-card">
      <SectionHeader icon={<IcoPalette />} title="Appearance Settings" sub="Customize your experience" iconBg="#d1fae5" iconColor="#10b981" />
      <div className="st-card-body">
        <div className="st-appear-row">
          ...
          <div className="st-theme-btns">
            <button type="button" className={`st-theme-btn${mode === 'light' ? ' active' : ''}`} onClick={() => setTheme('light')}>
              <IcoSun /> Light
            </button>
            <button type="button" className={`st-theme-btn${mode === 'dark' ? ' active' : ''}`} onClick={() => setTheme('dark')}>
              <IcoMoon /> Dark
            </button>
          </div>
        </div>
